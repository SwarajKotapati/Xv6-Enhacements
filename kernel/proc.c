...
{
  if(p->cow_enabled == 1 && p->cow_group != -1 ){
...
void track_heap(struct proc* p, uint64 start, int npages) {
  for (int i = 0; i < MAXHEAP; i++) {
    if (p->heap_tracker[i].addr == 0xFFFFFFFFFFFFFFFF) {//Review
      
      //printf("\nHey track_heap %d",p->heap_tracker[i].addr);
...
growproc(int n)
{
   ...
   */
   ...
   if(p->ondemand == true){
   	...
   	uint64 tsz = p->sz;
   	track_heap(p,tsz,n/4096);
   	
   	print_skip_heap_region(p->name,tsz,n/4096);
   }
   else{
...
fork(int cow_enabled)
{
  ...
  if(p->cow_enabled == 1){ // Children of a parent
     ...
     if(flag == 1){
     	cow_init(); // Initialize a cow group
     	printf("Cow init \n");
     	flag = 0;
     }
     int groupno = p->cow_group;
     // Initialize a group no 
     //printf("\n govindha  cow->group %d",p->cow_group);
     cow_group_init(groupno);
     
     incr_cow_group_count(p->cow_group);
     incr_cow_group_count(p->cow_group);
      
     //printf("Cow group init \n");
  }

  // implement and call the uvm_copy() function defined in cow.c
     uvmcopy_cow(p->pagetable, np->pagetable, p->sz);
     //printf("uvmcopy sucess");
  }
  
else{


     //printf("\nNon cow process Else part");
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  //printf("\nExit Out of condition with pid %d",pid);
